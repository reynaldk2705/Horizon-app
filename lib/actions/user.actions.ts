...
export const getUserInfo = async ({ userId }: getUserInfoProps) => {
  try {
    ...
    let user = await database.listDocuments(
      DATABASE_ID!,
      USER_COLLECTION_ID!,
      [Query.equal('userId', [userId])]
    );

    // Fallback to query by 'user_id' if schema uses underscore
    if (!user.documents || user.documents.length === 0) {
      user = await database.listDocuments(
        DATABASE_ID!,
        USER_COLLECTION_ID!,
...
export const exchangePublicToken = async ({
  ...
  try {
    if (!user || !user.$id) {
      console.error("exchangePublicToken error: User is missing or invalid.");
      return null;
    }
    ...
    try {
      ...
      console.warn("Dwolla funding source creation skipped or failed:", dwollaError);
    }
